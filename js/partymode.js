...
const partyWords = [
  "Harry Hops Hotline: " + c.phone(),
  "Einhorn Klebestift!",
  "𝑓ancy 𝑓!",
  "Party auf KRDB?",
  "Richtig fancy",
  "Phonk gefällig?",
  "DJ Autoplay hat reingaggt!",
  "Rena ist dumm!",
  "9/10 DJs empfehlen dieses Lied!",
];
const audio = new Audio(
  "https://download.scamcraft.net/Dubtec Records - Audio K9 - Clubbed To Tech (Extended Mix).m4a",
);
let loopInterval;
let index = 1;
...
function stopparty() {
  ...
  document
    .getElementById("feed-party-grab")
    .classList.remove("rainbow-animated");
...
function swaptext() {
  if (index === 1) {
    ...
    document.getElementById("feed-party-grab").innerHTML =
      "KRDB.INFO: Party Mode!";
    index = 2;
    return true;
  }
  if (index === 2) {
    ...
    if (d.getFullYear() - 2025 == "1") {
      document.getElementById("feed-party-grab").innerHTML =
        "Anniversary (" + (d.getFullYear() - 2025) + " Jahr)";
    } else {
      document.getElementById("feed-party-grab").innerHTML =
        "Anniversary (" + (d.getFullYear() - 2025) + " Jahre)";
    }
    index = 1;
    return true;
  }
